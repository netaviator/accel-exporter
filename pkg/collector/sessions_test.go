...
----------+------------+------------+--------------+--------------+---------+--------
 l66-54590#l66@bsa-vdsl | 102400/40960 | 122529 | 850000000 | 18000000000 | 900000 | 1200000
 l66-54584#l66@bsa-ftth |  | 57903 | 100 | 200 | 3 | 4
`
...
func TestSessionCountersAreLabelledByUsernameAndRealm(t *testing.T) {
	...
	user := map[string]string{"username": "l66-54590#l66@bsa-vdsl", "realm": "l66@bsa-vdsl"}
...
func TestUnshapedSessionsHaveNoRateLimitSeries(t *testing.T) {
	...
	for _, name := range []string{"accel_session_rx_rate_limit_bytes_per_second", "accel_session_tx_rate_limit_bytes_per_second"} {
		for _, m := range fams[name].GetMetric() {
			if labelsOf(m)["username"] == "l66-54584#l66@bsa-ftth" {
...
func TestDuplicateUsernamesYieldOneSeriesAndAreCounted(t *testing.T) {
	dup := sampleSessions + " l66-54590#l66@bsa-vdsl | 1/1 | 5 | 9 | 9 | 9 | 9\n"
	...
	kept := findSeries(t, mf, map[string]string{"username": "l66-54590#l66@bsa-vdsl"})
