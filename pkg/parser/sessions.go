...
// ("l66-54590#l66@bsa-vdsl" -> "l66@bsa-vdsl"), or "none" without one.
...
func CollectSessions(accelCmdPath string, timeout time.Duration) ([]Session, int, error) {
	...
	cmd := exec.CommandContext(ctx, accelCmdPath, "show", "sessions", sessionColumns)
	...
	sessions, parseErrors := parseSessions(out.String())
